// ============================================================
// Cloudflare KV 기반 데이터 저장소 (단일 JSON 문서)
// - 전체 DB를 하나의 KV 키("db")에 JSON으로 저장
// - 최초 실행 시 업종 표준 콘텐츠를 자동 시드
// - 요청 단위로 KV에서 로드하여 메모리 캐시 후, 변경 시 persist()로 KV에 기록
// ============================================================
...
// ---------- 타입 ----------
export type Theme = {
  navy900: string; navy800: string; navy700: string; navy600: string;
  primary: string; accent: string; ink: string; line: string; bg: string; bgSoft: string;
};

export const DEFAULT_THEME: Theme = {
  navy900: '#0A0F1E', navy800: '#0F1A33', navy700: '#143A6B', navy600: '#1E4E8C',
  primary: '#143A6B', accent: '#C8A04B', ink: '#16202E', line: '#E3E8EF',
  bg: '#FFFFFF', bgSoft: '#F5F7FA'
};
...
export type Settings = {
  ...
  heroPortraitUrl: string; // 히어로 우측 대표(프로필) 사진
  theme: Theme; stats: Stat[]; updatedAt: string;
};

export type ServiceCategory = { id: number; name: string; sortOrder: number };

export type Service = {
  id: number; categoryId: number; title: string; slug: string; icon: string;
  summary: string; overview: string; target: string; documents: string;
  process: string[]; priceInfo: string; faq: { q: string; a: string }[];
...
export type Inquiry = {
  ...
  agreePrivacy: boolean; status: '신규' | '연락완료' | '종결'; adminMemo: string;
  sourceIp: string; userAgent: string; createdAt: string; handledAt: string | null;
};

export type Post = {
  id: number; type: 'notice' | 'blog'; topic: string; title: string; content: string;
  thumbnailUrl: string; isPinned: boolean; isPublished: boolean; viewCount: number;
  publishedAt: string; createdAt: string; updatedAt: string;
};

export type AdminUser = {
  username: string; passwordHash: string; failedAttempts: number;
...
// 요청 단위 메모리 캐시 (동일 요청 내 반복 KV 읽기 방지)
const g = globalThis as unknown as { __knsdb?: DB };

// KV 바인딩 획득 (OpenNext 실행 컨텍스트)
function kv(): KVNamespace {
  const { env } = getCloudflareContext();
  const ns = (env as unknown as { KNS_DB?: KVNamespace }).KNS_DB;
  if (!ns) throw new Error('KV binding KNS_DB is not configured');
  return ns;
}

// 전체 DB 로드 (없으면 시드 후 저장)
export async function db(): Promise<DB> {
  if (g.__knsdb) return g.__knsdb;
  const ns = kv();
  const raw = await ns.get(KV_KEY);
  if (raw) {
    g.__knsdb = JSON.parse(raw) as DB;
  } else {
    g.__knsdb = seedData();
    await ns.put(KV_KEY, JSON.stringify(g.__knsdb));
  }
  return g.__knsdb;
}
...
export async function persist(): Promise<void> {
  if (!g.__knsdb) return;
  await kv().put(KV_KEY, JSON.stringify(g.__knsdb));
}

export async function nextId(coll: string): Promise<number> {
  const d = await db();
  d.seq[coll] = (d.seq[coll] || 0) + 1;
  return d.seq[coll];
}
...
// ---------- 최초 시드 (업종 표준 콘텐츠 - 관리자 모드에서 자유롭게 교체) ----------
function seedData(): DB {
  ...
  const categories: ServiceCategory[] = [
    { id: 1, name: '외국인·국제 업무', sortOrder: 0 },
    { id: 2, name: '인허가·신고', sortOrder: 1 },
    { id: 3, name: '법인·사업 지원', sortOrder: 2 },
    { id: 4, name: '권리구제·서류작성', sortOrder: 3 }
  ];

  const sv = (o: Partial<Service> & { id: number; categoryId: number; title: string; slug: string }): Service => ({
    icon: '📋', summary: '', overview: '', target: '', documents: '', process: [],
    priceInfo: '상담 후 안내', faq: [], sortOrder: 0, isPublished: true,
    createdAt: t, updatedAt: t, ...o
  });

  const services: Service[] = [
    sv({
      id: 1, categoryId: 1, sortOrder: 0, title: '외국인 비자·체류', slug: 'visa', icon: '🛂',
      ...
      process: ['무료 상담·체류자격 진단', '요건 검토 및 서류 안내', '신청서 작성·접수 대행', '출입국청 심사 대응', '허가 수령·사후 안내'],
      faq: [
    ...
    sv({
      id: 2, categoryId: 1, sortOrder: 1, title: '외국인 초청·사증발급인정', slug: 'invitation', icon: '✉️',
      ...
      process: ['상담·초청 요건 확인', '서류 준비 안내', '사증발급인정서 신청', '심사 대응', '인정서 발급·현지 비자 신청 안내'],
      faq: [{ q: '초청까지 얼마나 걸리나요?', a: '통상 접수 후 2~4주가 소요되며, 자격·심사 상황에 따라 달라질 수 있습니다.' }]
    }),
    sv({
      id: 3, categoryId: 1, sortOrder: 2, title: '귀화·국적 취득', slug: 'naturalization', icon: '🇰🇷',
      ...
      process: ['요건 진단 상담', '서류 수집·번역 공증', '귀화 신청 접수', '면접·심사 대비', '국적 취득·후속 절차'],
      faq: [{ q: '귀화 심사 기간은 얼마나 되나요?', a: '유형에 따라 다르나 통상 1년 내외가 소요됩니다. 서류 완성도가 기간 단축의 핵심입니다.' }]
    }),
    sv({
      id: 4, categoryId: 2, sortOrder: 0, title: '영업 인허가', slug: 'business-license', icon: '📑',
      ...
      process: ['업종·입지 검토', '요건·시설 기준 확인', '신청서 작성·접수', '현장 실사 대응', '허가증 수령'],
      priceInfo: '업종별 상이, 상담 후 안내',
      faq: [{ q: '입지가 허가 요건에 맞는지 먼저 알 수 있나요?', a: '네. 계약 전 입지·용도 검토를 먼저 해드립니다. 계약 후 불허 사례를 예방하는 가장 중요한 단계입니다.' }]
    }),
    sv({
      id: 5, categoryId: 2, sortOrder: 1, title: '각종 신고·등록 대행', slug: 'report', icon: '🗂️',
      ...
      process: ['상담·요건 확인', '서류 작성', '접수 대행', '수리·등록 확인']
    }),
    sv({
      id: 6, categoryId: 3, sortOrder: 0, title: '법인·사업자 설립', slug: 'incorporation', icon: '🏢',
      ...
      process: ['설립 구조 상담', '정관·서류 작성', '설립 등기 연계', '사업자등록·인허가', '외투 등록(해당 시)'],
      faq: [{ q: '외국인도 국내 법인을 만들 수 있나요?', a: '가능합니다. 외국인투자촉진법에 따른 절차와 체류자격(D-8 등)까지 함께 설계해 드립니다.' }]
    }),
    sv({
      id: 7, categoryId: 3, sortOrder: 1, title: '정부지원·기업인증', slug: 'certification', icon: '🏅',
      ...
      process: ['기업 진단', '적합 인증·사업 매칭', '신청서·증빙 준비', '접수·심사 대응', '인증 취득']
    }),
    sv({
      id: 8, categoryId: 4, sortOrder: 0, title: '행정심판·이의신청', slug: 'appeal', icon: '⚖️',
      ...
      process: ['처분 내용 분석', '구제 전략 수립', '청구서·증거 작성', '심판 진행 대응', '재결·후속 조치'],
      priceInfo: '사안별 상담 후 안내',
      faq: [{ q: '행정심판 청구 기한이 있나요?', a: '처분이 있음을 안 날부터 90일, 처분이 있은 날부터 180일 이내입니다. 기한 경과 전 신속히 상담받으세요.' }]
    }),
    sv({
      id: 9, categoryId: 4, sortOrder: 1, title: '계약서·내용증명 작성', slug: 'documents', icon: '✍️',
      ...
      process: ['사실관계 상담', '문서 초안 작성', '검토·수정', '발송·후속 안내'],
      priceInfo: '문서 종류별 상담 후 안내'
    })
  ];

  const po = (o: Partial<Post> & { id: number; title: string }): Post => ({
    type: 'blog', topic: '일반', content: '', thumbnailUrl: '', isPinned: false,
    isPublished: true, viewCount: 0, publishedAt: t, createdAt: t, updatedAt: t, ...o
  });

  const posts: Post[] = [
    po({
      id: 1, type: 'notice', topic: '공지', isPinned: true,
      title: '케이앤에스 글로벌 행정사무소 홈페이지 오픈 안내',
      content: '<p>안녕하십니까. 케이앤에스 글로벌 행정사무소 홈페이지가 새롭게 문을 열었습니다.</p><p>서비스 안내, 진행 절차, 자료실, 온라인 상담신청 기능을 제공합니다. 많은 이용 바랍니다.</p>'
    }),
    po({
      id: 2, type: 'notice', topic: '공지',
      title: '상담 예약 안내 (평일 09:00~18:00)',
      content: '<p>전화(010-3547-1860) 또는 홈페이지 상담신청을 통해 예약하실 수 있습니다. 주말·공휴일은 사전 예약 시 상담 가능합니다.</p>'
    }),
    po({
      id: 3, topic: '비자·체류', title: 'E-7 특정활동 비자, 어떤 직종이 가능할까?',
    ...
    po({
      id: 4, topic: '비자·체류', title: '체류기간 연장, 언제부터 신청할 수 있나요?',
    ...
    po({
      id: 5, topic: '비자·체류', title: '결혼이민(F-6) 비자 준비 체크리스트',
    ...
    po({
      id: 6, topic: '귀화·국적', title: '간이귀화와 일반귀화, 무엇이 다른가요?',
    ...
    po({
      id: 7, topic: '인허가', title: '일반음식점 영업신고, 계약 전 확인할 3가지',
    ...
    po({
      id: 10, topic: '권리구제', title: '영업정지 처분을 받았다면? 행정심판으로 다투는 방법',
    ...
    po({
      id: 11, topic: '권리구제', title: '내용증명, 언제 어떻게 보내야 효과적일까?',
  ...
  return {
    ...
    seq: { categories: 4, services: 9, posts: 11, inquiries: 0 }
  };
}
